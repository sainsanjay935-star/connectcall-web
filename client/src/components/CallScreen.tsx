...
export default function CallScreen({ name, myVideoRef, userVideoRef, onEndCall, stream, remoteStream, callType = 'video' }: CallScreenProps) {
    ...
    const toggleVideo = () => {
        if (isAudioOnly) return; // Disable video toggle in audio-only calls
        if (stream) {
    ...
    return (
        <div className="fixed inset-0 z-[10000] bg-[#0b141a] flex flex-col overflow-hidden animate-in fade-in duration-500">
            {/* Top Bar */}
            <div className={`absolute top-0 left-0 right-0 z-20 transition-all duration-300 ${isVideoOff || isAudioOnly ? 'h-32 bg-[#075e54]/90' : 'h-24 bg-gradient-to-b from-black/60 to-transparent'}`}>
                ...
                {isVideoOff || isAudioOnly ? (
                    <div className="h-full w-full flex items-center justify-center p-4">
                        ...
                        <div className="relative z-10">
                            <div className="h-64 w-64 md:h-80 md:w-80 overflow-hidden rounded-full border-4 border-white/10 shadow-2xl bg-[#dfe5e7] dark:bg-[#2a3942] relative">
